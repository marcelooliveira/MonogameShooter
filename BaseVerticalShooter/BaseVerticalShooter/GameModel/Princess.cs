...
namespace Shooter.GameModel
{
    public class Princess : PhysicalObject
    {
        ...
        public Princess()
            : base()
        {
            Position = new Vector2(GameSettings.Instance.WindowTilesSize.X / 2 - Size.X / 2, 3);
        }
        ...
        public override void Update(GameTime gameTime, int tickCount, float scrollRows)
        {
            ...
            if ((deltaTicks / 8) % 2 == 1 && Position.Y < GameSettings.Instance.WindowTilesSize.Y / 2 - 1)
            {
                isWalking = true;
                Position = new Vector2(
                    GameSettings.Instance.WindowTilesSize.X / 2 - Size.X / 2
                        + (1f / (tileWidth * 8))
